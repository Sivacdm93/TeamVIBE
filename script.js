 ...
 * CONFIG
 *******************/
const STORE_URL = 'https://script.google.com/macros/s/AKfycbwCdOVlPuuZujR62T1lGoJKfi1MliaRWYISv-V9GPo9wIPFZt0p5s77lKC6u_N_mSxAQA/exec';
 ...
 * DATA
 *******************/
const items = [ /* ...your list exactly as posted... */ ];
...
function autoCats(title){
  const t = title.toLowerCase(); const got = [];
...
function updateCounts(){
  ...
  // 👇 these IDs must exist in index.html (fixed)
...
function makeCard(item){
  ...
  if(platform==='instagram'){
    ...
    mediaHTML = `<div class="media">
      <iframe width="100%" height="360" src="${ytEmbed(item.url)}" title="YouTube video" frameborder="0"
        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>
    </div>`;
  }else{
    mediaHTML = `<div class="media" style="padding:10px">
      <a class="btn primary" target="_blank" rel="noopener" href="${item.url}">Open link</a>
    </div>`;
  }
...
(async function init(){
  ...
  /* TITLE & QUOTE ANIMATIONS (optional if you keep page-level script) */
